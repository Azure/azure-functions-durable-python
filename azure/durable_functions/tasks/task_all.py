...
def task_all(tasks: List[Task]):
    ...
    if len(faulted) > 0:
        return TaskSet(is_completed, all_actions, results, is_faulted=True, exception=faulted[0], is_played=is_played)
    if is_completed:
        return TaskSet(is_completed, all_actions, results, False, complete_time, is_played=is_played)
    else:
