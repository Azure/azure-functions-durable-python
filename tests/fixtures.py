import pytest
from azure.durable_functions.models.DurableOrchestrationBindings \
    import DurableOrchestrationBindings
...
def get_binding_string():
    ...
    binding_string = '{"taskHubName":"TASK_HUB_NAME","creationUrls":{' \
                     ...
                     'functionName}[/{' \
                     'instanceId}]?code=AUTH_CODE",' \
                     '"createAndWaitOnNewInstancePostUri":"BASE_URL' \
                     '/orchestrators/{' \
                     'functionName}[/{instanceId}]?timeout={' \
                     'timeoutInSeconds}&pollingInterval={' \
                     'intervalInSeconds}&code=AUTH_CODE"},"managementUrls":{' \
                     '"id":"INSTANCEID",' \
                     '"statusQueryGetUri":"BASE_URL/instances/INSTANCEID' \
                     '?taskHub=TASK_HUB_NAME&connection' \
                     '=Storage&code=AUTH_CODE",' \
                     '"sendEventPostUri":"BASE_URL/instances/INSTANCEID' \
                     '/raiseEvent/{' \
                     'eventName}?taskHub=TASK_HUB_NAME&connection=Storage' \
                     '&code=AUTH_CODE",' \
                     '"terminatePostUri":"BASE_URL/instances/INSTANCEID' \
                     '/terminate?reason={' \
                     'text}&taskHub=TASK_HUB_NAME&connection=Storage&code' \
                     '=AUTH_CODE",' \
                     '"rewindPostUri":"BASE_URL/instances/INSTANCEID/rewind' \
                     '?reason={' \
                     'text}&taskHub=TASK_HUB_NAME&connection=Storage&code' \
                     '=AUTH_CODE",' \
