"""Comprehensive round-trip and validation tests for df_serialization.
...
class TestPrimitiveRoundTrips:
    ...
    def test_loose_with_matching_type(self, value, loose, caplog):
        # Use the actual type of the value as expected_type
...
class TestDictWithObjectProperty:
    ...
    def test_strict_encode_fails_for_nested_custom_objects(self, strict):
        ...
        encoded — json.dumps runs without default= so Hat raises TypeError."""
...
class TestStrictNoExpectedType:
    """In strict mode, df_loads without expected_type must never call import_module."""
...
class TestEdgeCases:
    ...
    def test_no_expected_type_no_warning(self, caplog):
        """When expected_type is None, no warnings should fire."""
        with caplog.at_level(logging.WARNING, logger=df_serialization.__name__):
