...
public class ErrorHandlingTests
{
    ...
    // NOTE: All of these tests differ from .NET as the error messages are different. Need to figure out a way for the tests to localize the
    // expected string outputs to the function language if this code is going to be globalized
    ...
    public async Task OrchestratorWithUncaughtActivityException_ShouldFail()
    {
        ...
        Assert.StartsWith("Orchestrator function 'rethrow_activity_exception' failed: Activity function 'raise_exception' failed: ", orchestrationDetails.Output);
        Assert.Contains("CustomException: This activity failed", orchestrationDetails.Output);
    }

    [Fact(Skip = "Not yet implemented in Python")]
    ...
    public async Task OrchestratorWithCaughtActivityException_ShouldSucceed()
    {
        ...
        var orchestrationDetails = await DurableHelpers.GetRunningOrchestrationDetailsAsync(statusQueryGetUri);
        Assert.StartsWith("Caught exception: Activity function 'raise_exception' failed: ", orchestrationDetails.Output);
        Assert.Contains("CustomException: This activity failed", orchestrationDetails.Output);
    }

    [Fact(Skip = "Skipped for Python as failure_details is a dotnet implementation detail")]
    ...
    public async Task OrchestratorWithRetriedActivityException_ShouldSucceed()
    {
        ...
        Assert.Contains(this.fixture.TestLogs.CoreToolsLogs, x => x.Contains("CustomException") &&
