...
$E2EAppVenvDirectory = Join-Path $E2EAppProjectDirectory ".venv"
...
function EnsureValidPythonVersion {
  ...
  if ($pythonVersionOutput -match 'Python (\d+)\.(\d+)\.(\d+)') {
      ...
      if ($major -ne 3 -or $minor -ne 11) {
          Write-Error "Python version is $major.$minor.$patch. Version 3.11 is required."
      } else {
          Write-Host "Python version is $major.$minor.$patch. All good!" -ForegroundColor Green
      }
  } else {
      Write-Error "Unable to parse Python version from output: $pythonVersionOutput"
  }
}
